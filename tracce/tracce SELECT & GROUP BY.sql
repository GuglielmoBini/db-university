...
SELECT *, FLOOR(DATEDIFF (CURDATE(), `date_of_birth`) / 365) as 'age' FROM `students` WHERE DATEDIFF(CURDATE(), `date_of_birth`) / 365 > 31;
...
SELECT COUNT(*) AS 'n. dipartimenti' FROM `departments`;

-- 8. Quanti sono gli insegnanti che non hanno un numero di telefono? (50)
SELECT COUNT(*) AS ' teachers without phone' FROM `teachers` WHERE `phone` IS NULL;
...
SELECT `office_address` AS 'indirizzo', COUNT(office_address) AS 'uffici prof' FROM `teachers` GROUP BY `office_address`;
